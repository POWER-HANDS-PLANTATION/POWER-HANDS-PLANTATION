...
try {
    ...
    console.log("🌍 Connected to the power hands plantation DB");
} catch {
...
let cc = Config.sessionName.replace(POWER-HANDS-PLANTATION;;;/gi, "").replace(POWER-HANDS-PLANTATION;;;/gi, ""); // DONT REMOVE BLACK;;; so you can connect BLACK md session id too
...
await updateStatus()
 ...
 Void.ev.on('group-participants.update', async(anu) => {
        ...
        Void.ev.on('connection.update', async(update) => {
                ...
                if (connection === 'open') {
                    console.log("✅ power hands Login Successful!");
                    console.log("⬇️ power hands Installing External Plugins...");
                    ...
                    for (let i = 0; i < check.length; i++) {
                     ...
                     console.log("😁  power hands External Plugins Installed!");fs.readdirSync(__dirname + "/../commands").forEach((plugin) => {
